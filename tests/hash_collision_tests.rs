...
fn collision_detected_skips_second_entity() {
    ...
    let (etype, eid): (String, String) = conn
        ...
        .unwrap();
    assert_eq!(
        etype, incumbent_type,
        "collision must not overwrite entity_type"
    );
    ...
    assert_eq!(
        count, 0,
        "colliding entity must not appear in search_entities"
    );
}
...
fn same_entity_idempotent() {
    ...
    search::populate_search_index(&conn, "feed", "f1", "Updated Name", "Updated Title", "", "")
        .unwrap();
...
fn collision_preserves_first_entity_search() {
    ...
    let _ =
        search::populate_search_index(&conn, collider_type, collider_id, "BetaTrack", "", "", "");
