...
async fn feed_query_exposes_publisher_rss_truth() {
    ...
    // ADR 0049 §5: the child feed has no itunes:author of its own, and its
    // owner name "Wavlake" names a platform, so release_artist falls to the
    // placeholder. It no longer borrows the linked publisher feed's artist
    // text.
    assert_eq!(child_feed_json["data"]["release_artist"], "Unknown Artist");
    assert_eq!(
        child_feed_json["data"]["release_artist_source"],
        "placeholder"
    );
    assert_eq!(
    ...
    // The track has no item author, so track_artist falls back to the feed
    // artist name (unchanged rule), which is now "Unknown Artist".
    assert_eq!(child_track_json["data"]["track_artist"], "Unknown Artist");
}
