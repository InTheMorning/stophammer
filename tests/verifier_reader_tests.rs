...
use stophammer::ingest::{IngestFeedData, IngestFeedRequest, IngestPaymentRoute, IngestTrackData};
...
fn verifier_write_attempt_blocked_on_reader() {
    ...
    assert!(
        result.is_err(),
        "chain should fail when verifier attempts a write"
    );
...
fn content_hash_verifier_works_on_reader() {
    ...
    assert!(
        result.is_err(),
        "matching hash should short-circuit as NO_CHANGE"
    );
    assert_eq!(
        result.unwrap_err().0,
        format!("[content_hash] {NO_CHANGE_SENTINEL}")
    );
}
