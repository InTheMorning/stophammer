...
fn test_app_state(db: Arc<Mutex<rusqlite::Connection>>) -> Arc<stophammer::api::AppState> {
    let signer = Arc::new(
        stophammer::signing::NodeSigner::load_or_create("/tmp/test-issue21.key")
...
fn sanitize_fts5_query_strips_unclosed_quotes() {
    ...
    assert!(!result.contains('"'), "unclosed quotes should be stripped: got {result}");
}

#[test]
fn sanitize_fts5_query_strips_fts5_operators() {
    let result = stophammer::search::sanitize_fts5_query("foo AND bar OR baz NOT qux");
    assert!(!result.contains("AND"), "AND should be stripped: got {result}");
    assert!(!result.contains("OR"), "OR should be stripped: got {result}");
    assert!(!result.contains("NOT"), "NOT should be stripped: got {result}");
}

#[test]
fn sanitize_fts5_query_strips_parentheses_and_asterisks() {
    let result = stophammer::search::sanitize_fts5_query("(hello*) world");
    assert!(!result.contains('('), "parens should be stripped: got {result}");
    assert!(!result.contains(')'), "parens should be stripped: got {result}");
    assert!(!result.contains('*'), "asterisks should be stripped: got {result}");
}

#[test]
fn sanitize_fts5_query_strips_near_operator() {
    let result = stophammer::search::sanitize_fts5_query("hello NEAR world");
    assert!(!result.contains("NEAR"), "NEAR should be stripped: got {result}");
}
...
async fn search_with_unclosed_quote_returns_400() {
    ...
    assert_ne!(resp.status(), 500, "malformed FTS5 query must not produce 500");
}
...
async fn search_with_fts5_operators_returns_200() {
    ...
    let resp = app.oneshot(req).await.expect("call handler");
    assert_ne!(resp.status(), 500, "FTS5 operators in user input must not produce 500");
}
...
async fn search_with_parentheses_returns_non_500() {
    ...
    let resp = app.oneshot(req).await.expect("call handler");
    assert_ne!(resp.status(), 500, "unmatched parentheses must not produce 500");
}
...
async fn search_with_normal_query_does_not_return_500() {
    ...
    assert_ne!(resp.status(), 500, "normal search query must not produce 500");
}
