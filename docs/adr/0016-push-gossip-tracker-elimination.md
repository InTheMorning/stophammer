...
Historical note: Authenticated access to `GET /sync/events` and `GET /sync/peers`
was decided later in ADR 0027. The bootstrap language below reflects the
original push/tracker decision as accepted at the time.
